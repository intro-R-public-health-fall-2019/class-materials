...
In Module 5, we will learn more about using R Markdown to create data analysis reports.  Some of the material from the module are adapted from "R for Data Science" by Grolemund and Wickham, chapter 27.  You can find this chapter [here](https://r4ds.had.co.nz/r-markdown.html).
...
There are a few packages that we will want to install for this module to help make pretty tables in our final report. You need to execute these lines by hand, since the `eval = FALSE` argument to the code chunk means these lines are not evaluated each time the document is compiled.
```{r , eval = FALSE}
...
The best way to learn tyhe formatting for R Markdown is simply to try them out; eventually they become second nature and you don't need to think about them.  If you forget, you can always consult the **Markdown Quick Reference** guide mentioned above!



# Code chunks

We've already been using code chunks to run code within an R Markdown document.  Think of each code chunk like a function; a chunk should be relatively self-contained and focused on a single task.  You don't want just one code chunk for your entire project, break your project into meaningful pieces and give each it's own chunk!
...
* `Cmd/Ctrl + Enter` runs the current line of code.
...
nmes.data %>%
  group_by(eversmk) %>%
  summarize(n = n(), mean.exp = mean(totalexp))
...
* Chunk names: Code chunks can be given an optional name: ```{r by-name}. Naming your code chunks lets you easily navigate to specific chunks using the drop-down code navigator in the bottom left of the script editor.  Also, when you produce a graphic with a code chunk with a useful names, it's easier to refer to them elsewhere in your report.

* Chunk options: Chunk output can be customised with options, arguments supplied to chunk header. There are almost 60 options that you can use to customize your code chunks. You can see the full list at [http://yihui.name/knitr/options/](http://yihui.name/knitr/options/). Here are some of the ones you'll use the most frequently in writing a report:
...
If you look at the top of this .Rmd file, you'll see the code chunk named `setup`.  This code chunk contains the function `knitr::opts_chunk$set()` which allows you to set the global options for all code chunks in the document.  This means that if you don't specify anything for an individual chunk, those are the options you will get for your chunk.  (This first code chunk is *always* named `setup`.)  You can see that in this case, the default is `echo = TRUE`, which means that the code will always be included with the output.  If we specified something different, like `echo = FALSE`, in an individual code chunk, it would override the global option and not show the code for that output.  What happens if you change to `echo = FALSE` in the setup chunk and reknit the file?  Try it!

## Making attractice tables
...
kable(exp_table, caption = "Medical expenditures by smoking status")
```
...
```{r}
kable(exp_table, 
      caption = "Medical expenditures by smoking status", 
      digits = 1,
...
The nice thing about `pander()` is that it can make nice tables out of things that aren't data frame; things like model output.  Let's play around with a simulated data set to illustrate a few things we can do with captions and the function `pander()`.
```{r }
...
pander(lm.D9, 
       caption = paste0("Linear model output for model weight ~ group, for n=", length(weight), " observations"))
```
...
```{r makefigcap}
figcap.nmes = paste0("Scatter plot of log10 medical expenditures vs age for n=", nrow(nmes.data), " observations")
```
...
Note that we can use options like `include = FALSE`, `echo = FALSE`, and `message = FALSE` in our code chunk headers to control what is printed out in the actual report.
...
You can control many other “whole document” settings by tweaking the parameters of the YAML header. You might wonder what YAML stands for: it’s “yet another markup language”, which is designed for representing hierarchical data in a way that’s easy for humans to read and write. R Markdown uses it to control many details of the output. Some of the things you can do here is to add a table of contents or a bibliography.

## Table of contents

If you look at the YAML header of this document, you can see that I've added a table of contents to my document by specifying some options under the `output:` section of the header.  I want my document to knit to HTML, so I have the `html_document:` specified and then under that I list some options I want to the HTML document:

* `number_sections: true` gives each section that's designated with a header (using `#`) a number according to a numbering scheme.  
* `toc: true` adds a clickable table of contents to the top of the document when it's knit.
